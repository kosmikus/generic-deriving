...
module Generics.Deriving.Base (
  ...
  -- |
  --
  -- Datatype-generic functions work by converting values of a certain type 'T' into
  -- corresponding values of a (nearly) isomorphic type 'Rep T'. The type 'Rep T' is
  -- built from a limited set of type constructors, all provided by this module. A
  -- datatype-generic function is supposed to be an overloaded function with instances
  -- for most of these type constructors. By performing the mapping between 'T' and
  -- 'Rep T', the generic instances can implement functionality that works for any
  -- representable type.
  --
  -- Representable types are collected in the 'Generic' class, that defined the
  ...
  -- use 'Rec0' everywhere.
  --
  -- This is a lot of information! Most of it is actually merely meta-information that makes
  -- the names of the datatype itself and the constructors as well as some additional information
  -- about these available on the type level, as well as information whether record syntax has
  -- been used or not.
  --
  ...
  --      @deriving Generic@. These datatypes are proxy types with no values. Their only
  --      purpose is that they are instances of the type class 'Constructor', which can be used
  --      in order to obtain information about the constructor in question, such as its name
  --      or infix priority.
  --
  --    * The @D1 D1Tree@ tag indicates that the enclosed part is the representation of the
  --      datatype 'Tree'. Again, 'D1Tree' is a datatype generated by the cmopiler. It is a
  --      proxy type, and its sole purpose is that it is an instance of class 'Datatype', which
  ...
  -- > type D1 = M1 D
  --
  -- The types 'S', 'C' and 'R' are once again type-level proxies, just used to differentiate
  -- the different variants of 'M1' if needed.
